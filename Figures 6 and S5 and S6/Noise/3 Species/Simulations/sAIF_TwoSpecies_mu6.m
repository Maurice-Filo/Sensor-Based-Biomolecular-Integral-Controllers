...
ProjectPath = '/cluster/home/mfilo/Fine Exhaustive Code Two Species';
DataPath = fullfile(ProjectPath, 'Data');
addpath(genpath(ProjectPath));
euler = parcluster('local');
pool = parpool(euler, 16);

% ProjectPath = '/Users/mfilo/Library/CloudStorage/GoogleDrive-maurice.g.filo@gmail.com/My Drive/Research at ETH/My Papers and Presentations/Bacterial PI/Matlab/Noise/Fine Exhaustive Code Two Species';
% DataPath = fullfile(ProjectPath, 'Data');
% addpath(genpath(ProjectPath));

%% Network Construction
StoichiometryMatrix = StoichiometryMatrix_sAIF_TwoSpecies();
PropensityFunction = @PropensityFunction_sAIF_TwoSpecies;
Parameters = Parameters_sAIF_TwoSpecies();
Output_Index = 2;
N_mu = 32;
...
%% Save data
Save_Flag = 1;
FileName = fullfile(DataPath, ['sAIF_TwoSpecies_delta_theta_eta_mu', num2str(Script_Index)]);
if Save_Flag == 1
    clear euler pool
    save(FileName);
end